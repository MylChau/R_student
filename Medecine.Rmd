...
What is the name of the first variable the model split on? Tphase
...
What is the training set accuracy of the CART model?Sensitivity?Specificity?
    FALSE TRUE
  0   631   99
  1   131  441
...
table(dtmTrain$trial,predTrain.prob>=0.5)
(631+441)/(631+99+131+441)#accuracy 0.8233487
(441)/(441+131)#sensitivity 0.770979
...
```{r}
predTest<-predict(trialCART,newdata=dtmTest,type="class")
table(dtmTest$trial,predTest)
...
```{r}
library(ROCR)
predTest.prob<-predict(trialCART,newdata=dtmTest)[,2]
predROCR<-prediction(predTest.prob,dtmTest$trial)
...
###Faire un graph treshold predROCR
```{r}
