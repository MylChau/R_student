...
table(test$isB)
(1175)/(383+1175)#accuracy=0.754172
```
...
(1118+340)/(1118+57+43+340)#accuracy 0.9358151
```
...
predForest<-predict(cartBforest,newdata=test)
table(test$isB,predForest)
(1165+374)/(1165+9+10+374)#accuracy 0.9878049
```
...
table(test2$letter)
(401)/(395+383+401+379)#accuracy: [1] 0.2573813
```
...
predCARTletter<-predict(CARTletter,newdata=test2,type="class")
table(test2$letter,predCARTletter)
(348+318+363+340)/nrow(test2)#accuracy [1] 0.8786906
...
```{r}
set.seed(1000)
lettertree<-randomForest(letter~.-isB,data=train2)
predlettertree<-predict(lettertree,newdata=test2)
table(test2$letter,predlettertree)
(390+380+393+364)/nrow(test2)#accuracy [1] 0.9801027
```
